#!/bin/bash
# if you get error when you run this script ,please input ":set fileformat=unix" then save this file to run 
echo "============================================"
...
# compilation hprose by vanthink version 


    mkdir hprose \
    && tar -xf hprose-pecl-vanthink.tar -C hprose --strip-components=1 \
    && (cd hprose && phpize && ./configure --enable-hprose=shared && make $mc && make install)\
    
# compilation php-xhprof-extension

    mkdir php-xhprof-extension \
    && tar -xf php-xhprof.tgz -C php-xhprof-extension --strip-components=1 \
